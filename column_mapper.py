...
# Helper function to convert raw string values for boolean-like fields
...
def map_confluence_columns_to_ml_ddl():
    ...
    # --- Column Mapping Loop ---
    report_lines = [] # For the column mapping report
    ...
    for page_row in pages_to_map:
        ...
        try:
            ...
            confluence_columns_to_map = [] # Columns from Confluence where 'add_source_to_target' is True
            all_current_confluence_target_names = set() # Track all target_field_names in current Confluence page
            
            # --- Extract Confluence columns and interpret 'add_source_to_target' ---
            for table_data in parsed_content.get('tables', []):
                if table_data.get('id') == 'table_1': # Only process 'table_1'
                    for column_detail in table_data.get('columns', []):
                        # Track for orphan detection (use its raw target_field_name)
                        all_current_confluence_target_names.add(column_detail.get('target_field_name')) 
                        
                        # Interpret 'add_source_to_target' field's string value (final logic)
            ...
            if not confluence_columns_to_map:
                ...
                # Still proceed to orphan cleanup even if no columns to map
            
            # Resolve the ML source FQDNs for this Confluence page
            ...
            if not first_source_table_from_conf:
                report_lines.append(f"  WARNING: No 'source_table' found in Confluence columns. Cannot resolve ML source. Skipping mapping for this page.")
                continue
            
            resolved_env_fqdns_map = fqdn_resolver_map.get(first_source_table_from_conf.upper())

            if not resolved_env_fqdns_map:
                report_lines.append(f"  WARNING: Confluence source_table '{first_source_table_from_conf}' not found in FQDN resolver map. Skipping mapping for this page.")
                continue
            ...
            for ml_env_upper in CHECK_ENVIRONMENTS:
                ...
                for conf_col_detail in confluence_columns_to_map:
                    ...
                    if existing_map_record and existing_map_record['user_override'] == 1:
                        report_lines.append(f"    - '{confluence_target_field_name}' -> '{existing_map_record['matched_ml_column_name']}' (USER OVERRIDDEN, {existing_map_record['match_percentage']}%). STATUS: {existing_map_record['mapping_status']}. (Active: {bool(existing_map_record['is_active'])})")
                        # Just update last_mapped_on and current DDL hash for audit (without changing user's choice)
                    ...
                    if existing_map_record:
                        # If DDL hash hasn't changed AND it was previously a good auto-mapping, skip re-matching
                        if existing_map_record['ml_source_ddl_hash_at_mapping'] == ml_ddl_info['current_ddl_hash'] and \
                           existing_map_record['mapping_status'] in ['MAPPED_EXACT', 'MAPPED_FUZZY']:
                            report_lines.append(f"    - '{confluence_target_field_name}' -> '{existing_map_record['matched_ml_column_name']}' ({existing_map_record['match_percentage']}%). STATUS: {existing_map_record['mapping_status']}. (Automated, DDL Unchanged)")
                            db_manager.insert_or_update_confluence_ml_column_map({
                                'confluence_page_id': confluence_page_id,
                                'confluence_target_field_name': confluence_target_field_name,
                                'ml_source_fqdn': ml_source_fqdn,
                    ...
                    if perform_fuzzy_match:
                        ...
                        if best_match_result:
                            ...
                            if exact_match_only and score < 100:
                                ...
                                new_mapping_data['notes'] = f"Auto-mapped: Fuzzy match ({score}%) below 100% exact_match_only threshold."
                                report_lines.append(f"    - '{confluence_target_field_name}' -> No exact match ({score}%). STATUS: {new_mapping_data['mapping_status']}")
                            elif score == 100:
                                new_mapping_data['mapping_status'] = 'MAPPED_EXACT'
                                new_mapping_data['notes'] = f"Auto-mapped: Exact match found for '{confluence_target_field_name}'."
                                report_lines.append(f"    - '{confluence_target_field_name}' -> '{matched_ml_col_name}' (Exact Match). STATUS: {new_mapping_data['mapping_status']}")
                            else:
                                new_mapping_data['mapping_status'] = 'MAPPED_FUZZY'
                                new_mapping_data['notes'] = f"Auto-mapped: Fuzzy match ({score}%) for '{confluence_target_field_name}' to '{matched_ml_col_name}'."
                                report_lines.append(f"    - '{confluence_target_field_name}' -> '{matched_ml_col_name}' ({score}%). STATUS: {new_mapping_data['mapping_status']}")
                        else:
                            new_mapping_data['mapping_status'] = 'UNMAPPED_LOW_SCORE'
                            new_mapping_data['notes'] = f"Auto-mapped: No match found above threshold ({match_threshold}%)."
                            report_lines.append(f"    - '{confluence_target_field_name}' -> No match found. STATUS: {new_mapping_data['mapping_status']}")
            ...
            for orphan_row in cursor.fetchall():
                ...
                if orphan_target_field_name not in current_target_field_names_in_conf:
                    ...
                    if orphan_user_override == 1:
                        report_lines.append(f"  INFO: Orphan detected for '{orphan_target_field_name}', but skipping deactivation due to user_override.")
                        db_manager.insert_or_update_confluence_ml_column_map({
                            'confluence_page_id': confluence_page_id,
                        ...
                        report_lines.append(f"  WARNING: Orphan mapping detected: '{orphan_target_field_name}' from page {confluence_page_id} is no longer in Confluence content. Marking as inactive.")
                        db_manager.insert_or_update_confluence_ml_column_map({
                            'confluence_page_id': confluence_page_id,
                            'confluence_target_field_name': orphan_target_field_name,
                            'ml_source_fqdn': ml_source_fqdn,
